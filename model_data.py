...
def model_data(df: pd.DataFrame, window_size=3, causal=True, use_constant=False, ark = False, max_lag=144):
    ...
    if window_size == 1:
        ...
        ar_features = ([f'lfp_{hemis[0]}_z_scored_{model}_lag_1'] + (['constant'] if use_constant else [])) if not ark else model_utils.select_sig_lags_kfold()
        ...
        if ark:
            ...
            for hemi in hemis:
                ...
                def process_group(group):
                    try:
                        sig_lags = model_utils.select_significant_lags_kfold(group, all_lags, target)
                        sig_lags = model_utils.drop_sparse_lags(group, all_lags, threshold=0.6)
                        if use_constant:
                            sig_lags.append('constant')
                        return model_utils.apply_sliding_window(group, sig_lags, target, window_size=window_size, causal=causal)
                ...
                try:
                    ...
                    ar_features = [f'lfp_{hemi}_z_scored_{model}_lag_1'] + (['constant'] if use_constant else [])
                    results_df = pt_groups.apply(lambda g: model_utils.apply_sliding_window(g, ar_features, f'lfp_{hemi}_z_scored_{model}', window_size=window_size, causal=causal), include_groups=False)
                # Merge predictions back
                df_w_preds = pd.merge(df_w_preds, results_df, how='outer', left_index=True, right_index=True)
