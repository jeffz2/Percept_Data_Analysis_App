...
def clean(raw_df, patient_name):
    if patient_name == "B017":
        raw_df.query('pt_id != "B017" or source_file != "Report_Json_Session_Report_017_01R_20240516T152238.json"', inplace=True)

        # B017's data is labeled differently from other OFC patients --> correct it to match the others.
        right_chest_jsons_017 = list(Path("Z:/PerceptOCD-48392/017/LFP/R").glob('[!.]*.json'))
    ...
    pt = [patient_name]
    ofc_pt_right_ipg_data_vcvs = raw_df.query('pt_id in @pt and left_lead_location == "OTHER" and right_lead_location == "VC/VS"').copy()
    ...
    ofc_pt_left_ipg_data_vcvs = raw_df.query('pt_id in @pt and left_lead_location == "VC/VS" and right_lead_location == "OTHER"').copy()
