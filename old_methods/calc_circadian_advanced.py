...
def load_mat_data(mat_file: list, pt_id: int, hemi: int):
    ...
    df = pd.DataFrame(
        percept_data["LFP_filled_matrix"][pt_name][hemi],
        index=TIME_INDEX,
        columns=day_col,
    )
    ...
    se_df = pd.DataFrame(
        (percept_data["entropy"][pt_name][hemi]).reshape(1, -1),
        index=["SE"],
        columns=day_col,
    )
...
def get_state_labels(mat_file: list, pt_id: int, master_df: pd.DataFrame):
    ...
    for label, state in enumerate(["hypomania", "non_responder", "responder"]):
        ...
        state_label[
            [
                i
                for i, date in enumerate(master_df.columns)
                if date.days in np.array(state_days)
            ]
        ] = (label + 1)
...
def create_dataloader(
    X_df: pd.DataFrame, y_df: pd.DataFrame, batch_size: int = 64, shuffle: bool = True
):
    """
    ...
    X = (
        X_df.values.T
    )  # Transpose to match expected input dimensions for PyTorch models.
    y = y_df.values
...
def run_regression(master_df: pd.DataFrame, lag_terms: list):
    ...
    significant_lags_per_fold = (
        []
    )  # List to track significant lags identified in each fold
    ...
    final_model = sm.OLS(
        master_df.loc["Original Data"], master_df.loc[common_lags].T
    ).fit()

    # Determine if all terms in the final model are significant
    cont = (
        0 if (final_model.pvalues < 0.05).sum() >= len(final_model.pvalues) - 1 else 1
    )
...
def NN_AR(train: pd.DataFrame, test: pd.DataFrame, index: list):
    ...
    test_loader = create_dataloader(
        test.loc[index, :], test.loc["Original Data", :], shuffle=False
    )
    ...
    preds = metrics["best_predictions"][
        1
    ]  # Predictions corresponding to the best R-squared value.
...
def cosinor(train: pd.DataFrame, test: pd.DataFrame, comp: int):
    ...
    exogs = (
        [f"rrr{i}" for i in range(1, comp + 1)]
        + [f"sss{i}" for i in range(1, comp + 1)]
        + ["constant"]
    )
...
def get_r2(
    ...
    for test_day in days:
        ...
        r2_df.loc[r2_df.shape[0]] = [
            test_day,
            r2_score(clean_orig, clean_pred),
            state,
            LABEL_MAPPING.get(state, 3),
        ]
...
def compile_data(mat_file: str, pt_id: int, hemi: int, components: list):
    ...
    master_df = pd.DataFrame(
        columns=[
            timedelta(days=day, hours=t.hour, minutes=t.minute, seconds=t.second)
    ...
    master_df.loc["contig"] = [
        c
        for c in range(len(contig) - 1)
        for _ in range(contig[c] * 144, contig[c + 1] * 144)
    ]
    ...
    for lag in range(1, 145):
        ...
        for ch in set(master_df.loc["contig"].tolist()):
            lag_data.extend(
                master_df.loc["Original Data", master_df.loc["contig"] == ch].shift(
                    periods=lag
                )
            )
    ...
    for i in range(1, components[pt_id] + 1):
        master_df.loc[f"rrr{i}"] = np.cos(
            2
            * np.pi
            * np.array(
                [
                    (master_df.columns[x].seconds) / 600
                    for x in range(len(master_df.columns))
                ]
            )
            / (144 / i)
        )
        master_df.loc[f"sss{i}"] = np.sin(
            2
            * np.pi
            * np.array(
                [
...
def compile_r2(
    ...
    for state in set(master_df.loc["State_Label"]):
        ...
        if model == "SE":
            ...
            ci_inter = st.t.interval(
                confidence=0.95,
                df=len(state_se) - 1,
                loc=np.mean(state_se),
                scale=st.sem(state_se),
            )
            ...
            if gl.shape[1] > 145:
                r2_df, pred_df, kfold_r2 = get_r2(
                    gl, r2_df, state, pred_df, df, index, model, components[pt[0]]
                )
                ...
                ci_inter = st.t.interval(
                    confidence=0.95,
                    df=len(kfold_r2) - 1,
                    loc=np.mean(kfold_r2),
                    scale=st.sem(kfold_r2),
                )
                ci_info[pt[1]][f"{label_key[state]}"] = str(ci_inter)
...
def main(
    ...
    for model in models:
        ...
        for pt_id, pt_name in zip(pt_index, pt_names):
            ...
            r2_info, ci_info, log_df = compile_r2(
                df,
                master_df,
                se,
                log_df,
