...
def generate_data(
    ...
    if file_list is None:
        file_list = filedialog.askopenfilenames(
            title="Select patient JSON files", filetypes=[("JSON files", "*.json")]
        )
    ...
    raw_data["Timestamp"] = pd.to_datetime(raw_data["Timestamp"]).dt.tz_convert(
        time_zone
    )
    ...
    time_of_day_minutes = (unique_dates - unique_dates.dt.floor("D")).view("int64") / (
        60 * 1e9
    )
    ...
    LFP_matrix = {
        1: np.full((144, len(unique_rounded_dates)), np.nan),
        2: np.full((144, len(unique_rounded_dates)), np.nan),
    }
    stim_matrix = {
        1: np.full((144, len(unique_rounded_dates)), np.nan),
        2: np.full((144, len(unique_rounded_dates)), np.nan),
    ...
    time_matrix = {
        1: np.full((144, len(unique_rounded_dates)), np.nan, dtype="datetime64[ns]"),
        2: np.full((144, len(unique_rounded_dates)), np.nan, dtype="datetime64[ns]"),
    }
    ...
    for i, unique_date in enumerate(unique_rounded_dates):
        ...
        LFP_matrix[1][disc_TOD_radix[idx], i] = raw_data.iloc[idx][
            "LFP Amp Left"
        ].values
        time_matrix[1][disc_TOD_radix[idx], i] = unique_dates.iloc[idx].values
        stim_matrix[1][disc_TOD_radix[idx], i] = raw_data.iloc[idx][
            "Stim Amp Left"
        ].values

        LFP_matrix[2][disc_TOD_radix[idx], i] = raw_data.iloc[idx][
            "LFP Amp Right"
        ].values
        time_matrix[2][disc_TOD_radix[idx], i] = unique_dates.iloc[idx].values
        stim_matrix[2][disc_TOD_radix[idx], i] = raw_data.iloc[idx][
            "Stim Amp Right"
        ].values
    ...
    for hemisphere in [1, 2]:
        ...
        time_matrix_naive_flat = pd.to_datetime(
            time_matrix[hemisphere].ravel()
        ).to_numpy()

        DBS_time[hemisphere] = np.round(
            (unique_cal_days_naive - DBS_onset_naive).total_seconds() / 86400
        ).to_numpy()
        time_matrix_relative = (
            time_matrix_naive_flat - np.datetime64(DBS_onset_naive)
        ) / np.timedelta64(1, "D")
        time_matrix_relative[np.isnat(time_matrix_naive_flat)] = np.nan
        time_matrix[hemisphere] = time_matrix_relative.reshape(
            time_matrix[hemisphere].shape
        )
        ...
        LFP_norm_matrix[hemisphere] = (
            LFP_matrix[hemisphere] - np.nanmean(LFP_matrix[hemisphere], axis=0)
        ) / np.nanstd(LFP_matrix[hemisphere], axis=0, ddof=1)
        filled = utils.fill_data(LFP_matrix[hemisphere], DBS_time[hemisphere])
        LFP_filled_matrix[hemisphere] = (
            filled - np.nanmean(filled, axis=0)
        ) / np.nanstd(filled, axis=0, ddof=1)
