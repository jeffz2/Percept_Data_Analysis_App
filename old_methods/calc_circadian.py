...
def calc_circadian(
    ...
    for subject in percept_data["LFP_norm_matrix"].keys():
        ...
        for hemisphere in [0, 1]:
            ...
            start_index = (
                np.concatenate(([0], start_index + 1, [len(days)]))
                if len(start_index)
                else np.array([0, len(days)])
            )
            ...
            for i in range(len(days)):
                if (
                    any(
                        (start_index > i - cosinor_window_left)
                        & (start_index <= i + cosinor_window_right)
                    )
                    or len(days) < i + cosinor_window_right
                ):
                    continue
                y = LFP_norm[
                    :, (i - cosinor_window_left) : (i + cosinor_window_right + 1)
                ].T.flatten()
                t = time_matrix[
                    :, (i - cosinor_window_left) : (i + cosinor_window_right + 1)
                ].T.flatten()
                amp[i, :num_peaks], acro[i, :num_peaks], p_values[i], _ = utils.cosinor(
                    t, y, 24, num_components, num_peaks
                )
            ...
            for i in range(len(days)):
                print(
                    f"{subject} - {i + 1}"
                )  # Used for logging disregard if not needed
                if any((start_index > i) & (start_index <= i)) or len(days) < i:
                    continue
                y = LFP_norm[:, i].T.flatten()
                t = time_matrix[:, i].T.flatten()
                _, template_acro[i, :num_peaks], template_p[i], _ = utils.cosinor(
                    t, y, 24, num_components, num_peaks
                )
    ...
    print(
        "Performing advanced circadian calculations..."
    )  # Used for logging disregard if not needed
    ...
    percept_data = utils.merge_advanced_data(
        percept_data=percept_data, advanced_data=advanced_calc_data
    )
