...
def run_regression(df: pd.DataFrame, pts: list, delta: bool, features: list, 
                   val_type: str, num_test: float = None, shuffle: bool = False, plot: bool = False, 
    ...
    if val_type == 'LOPO':
        if plot_thresh:
            fig, axs = plt.subplots(nrows=np.ceil(len(pts) / 2).astype(int), ncols=2, figsize=(12,np.ceil(len(pts) / 2).astype(int)*3), sharey=True)
        ...
        for i, pt in enumerate(pts):
            ...
            if plot_thresh:
                axs.flatten()[i], threshold = plot_regression_threshold(axs.flatten()[i], pt, raw_df, model, features, delta)
    ...
    # Single leave one patient out fold
        ...
        if plot:
            ax, threshold = plot_regression_threshold(ax, val_type, raw_df, model, features, delta)
            fig.tight_layout()
...
def plot_regression_threshold(ax, pt, df, model, features, delta):
    ...
    for f in features:
        ...
        for i in range(len(start_index)-1):
            ...
            ax.plot(seg_days, df.query('pt_id == @pt and days_since_dbs in @seg_days')[f].rolling(window=5, min_periods=1).mean(), color='lightgray', lw=2, alpha=1)
    ...
    if pt in ['B001', 'B004', 'B005', 'B011', 'B013']:
        ...
        cross_inds = np.where(df.query('pt_id == @pt')[f].rolling(window=5, min_periods=1).mean() <= (threshold))[0]
        i = 0
        day_cross = -1
        ...
        while day_cross <= 0:
            ...
            i += 1
            day_cross = df.loc[df.query('pt_id == @pt')['days_since_dbs'].index[cross_ind], 'days_since_dbs']

        ax.vlines(day_cross, ymin=-0.5, ymax=1, color='red', ls='--', lw=2, label=f'{day_cross}')
        #print(f'{pt}: {day_cross}')

        with open('data/param.json', 'r') as f:
            params = json.load(f)[pt]
            ax.vlines(params['responder_day'], ymin=-0.5, ymax=1, color='green', ls='--', lw=2)

    ax.set(xlabel='Days since DBS', ylabel='R²', title=f'{pt}: {np.round(threshold, 4)}', xlim=ax.get_xlim(), ylim=(-0.5, 1) if not delta else (-1, 0.5))
    return ax, threshold 